...
-- Composite index to optimize `get_recent_approved_posts` query by indexing multiple filters and pre-sorting on created_at DESC
...
-- Composite index to optimize `get_pending_draft_for_customer` query by indexing multiple filters and pre-sorting on created_at DESC
