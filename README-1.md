...
1. **In `googleAuth.ts`**, find wherever tokens are read/written (likely a
   ...
   - **(A) HTTP call**: expose a tiny internal endpoint on this FastAPI
     backend (e.g. `GET/POST /internal/gbp-credentials`) that
     `googleAuth.ts` calls instead of touching a local file. Simplest to
     wire up, adds one network hop per token read.
