...
export class ReviewService implements IReviewService {
    ...
    /**
     * Returns lazily-created googleapis clients scoped to one location.
     * Called at the top of each method instead of once in the constructor,
     * so every call gets a fresh auth credential for the right location.
     */
    private async getClientsForLocation(locationId: string) {
        const auth = await this.authService.getAuthenticatedClient(locationId);
        return {
            mybusinessbusinessinformation: google.mybusinessbusinessinformation({ version: 'v1', auth }),
            mybusinessaccountmanagement: google.mybusinessaccountmanagement({ version: 'v1', auth }),
        };
    }
     ...
     * @private
     */
    private async resolveLocationPath(locationName: string, locationId: string): Promise<string> {
        if (locationName.includes('accounts/')) {
            return locationName;
        }
        
        const account = await this.apiClient.getFirstAccount(locationId);
        return buildFullLocationPath(locationName, account.name);
    }
    
    /**
     * List all business locations accessible via a given location's credentials.
     * `locationId` is required because there is no longer a single global auth context —
     * it identifies which customer's credentials to use for this API call.
     */
    async listLocations(locationId?: string): Promise<ServiceResponse<ListLocationsResponse>> {
        try {
            const id = locationId ?? '';
            if (!id) {
                return { success: false, error: 'listLocations requires a locationId — no global auth context exists in multi-tenant mode.' };
            }
            const { mybusinessaccountmanagement, mybusinessbusinessinformation } = await this.getClientsForLocation(id);
            ...
            for (const account of accounts) {
                ...
                try {
                    const locationsResponse: any = await mybusinessbusinessinformation.accounts.locations.list({
                        parent: accountName,
     ...
     * `locationName` should be the full GBP resource path (accounts/{a}/locations/{l})
     * so that the locationId can be extracted from it for token lookup.
     */
    async getReviews(locationName: string, pageSize = DEFAULTS.PAGE_SIZE, pageToken?: string): Promise<ServiceResponse<GetReviewsResponse>> {
        try {
            // Extract locationId from the full resource path when available
            ...
            // Build the full path
            const fullLocationPath = await this.resolveLocationPath(locationName, locationId);
    ...
    async getBusinessProfile(locationName?: string): Promise<ServiceResponse<BusinessProfile>> {
        try {
            const nameForLookup = locationName ?? '';
            const locationId = nameForLookup.includes('locations/')
                ? nameForLookup.split('locations/')[1]
                : nameForLookup;

            if (!locationId) {
                return {
                    success: false,
                    error: 'getBusinessProfile requires a locationName — no global auth context exists in multi-tenant mode.',
                    errorCode: ERROR_CODES.NO_LOCATIONS_ERROR
                };
            }

            const { mybusinessbusinessinformation } = await this.getClientsForLocation(locationId);
            
            // Build full path
            const fullLocationPath = await this.resolveLocationPath(nameForLookup, locationId);
            logPathResolution(nameForLookup, fullLocationPath, 'getBusinessProfile');
            
            logger.debug(`Fetching business profile for location: ${fullLocationPath}`);
            
            const response: any = await (mybusinessbusinessinformation as any).locations.get({
